...
type TransformControlsEventMap = {
  'change': {},
  'mousedown': { mode: TransformModeEnum; },
  'mouseup': { mode: TransformModeEnum; },
  'objectchanged': {};
};

/**
 * TODO:
 * I really need to refactor this, but i've no idea how it works
 *
 * copied directly from the three.js lib
 */
class TransformControls extends Controls<TransformControlsEventMap> {
  ...
  pointerMove(pointer) {
    ...
    this.dispatchEvent({ type: 'objectchanged' });
  }
  ...
  updateMatrixWorld(force) {
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      if (this.mode === 'translate' || this.mode === 'scale') {
        ...
        const AXIS_HIDE_THRESH = 0.99;
        const PLANE_HIDE_THRESH = 0.2;

        if (handle.name === 'X') {
          if (Math.abs(_alignVector.copy(_unitX).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_THRESH) {
        ...
        if (handle.name === 'Y') {
          if (Math.abs(_alignVector.copy(_unitY).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_THRESH) {
            handle.scale.set(1e-10, 1e-10, 1e-10);
            handle.visible = false;
          }
        }
        ...
        if (handle.name === 'Z') {
          if (Math.abs(_alignVector.copy(_unitZ).applyQuaternion(quaternion).dot(this.eye)) > AXIS_HIDE_THRESH) {
            handle.scale.set(1e-10, 1e-10, 1e-10);
            handle.visible = false;
          }
        }
        ...
        if (handle.name === 'XY') {
          if (Math.abs(_alignVector.copy(_unitZ).applyQuaternion(quaternion).dot(this.eye)) < PLANE_HIDE_THRESH) {
            handle.scale.set(1e-10, 1e-10, 1e-10);
            handle.visible = false;
          }
        }
        ...
        if (handle.name === 'YZ') {
          if (Math.abs(_alignVector.copy(_unitX).applyQuaternion(quaternion).dot(this.eye)) < PLANE_HIDE_THRESH) {
            handle.scale.set(1e-10, 1e-10, 1e-10);
            handle.visible = false;
          }
        }
        ...
        if (handle.name === 'XZ') {
          if (Math.abs(_alignVector.copy(_unitY).applyQuaternion(quaternion).dot(this.eye)) < PLANE_HIDE_THRESH) {
            handle.scale.set(1e-10, 1e-10, 1e-10);
            handle.visible = false;
          }
        }
